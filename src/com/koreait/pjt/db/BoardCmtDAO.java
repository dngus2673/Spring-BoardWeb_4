...
public class BoardCmtDAO {
	...
	public static List<BoardCmtDomain> selCmtList(int i_board) {
		...
		// 수정과 삭제에 필요하기 때문에 i_user이 필요
		String sql = " SELECT A.i_user, A.cmt, A.r_dt, A.m_dt, A.i_cmt, B.nm "	
				+ " FROM t_board4_cmt A "
				+ " INNER JOIN t_user B "
				+ " ON A.i_user = B.i_user "
				+ " WHERE A.i_board = ? "
				+ " ORDER BY A.i_cmt ";
			// string 값보다 정수 값이 빠르기 때문
		JdbcTemplate.executeQuery(sql, new JdbcSelectInterface() {
			...
			public int executeQuery(ResultSet rs) throws SQLException {
				while(rs.next()) {
					...
					BoardCmtDomain cvo = new BoardCmtDomain();
					cvo.setCmt(rs.getNString("cmt"));
					cvo.setI_user(rs.getInt("i_user"));
					cvo.setI_cmt(rs.getInt("i_cmt"));
					cvo.setR_dt(rs.getNString("r_dt"));
					cvo.setM_dt(rs.getNString("m_dt"));
					cvo.setNm(rs.getNString("nm"));
	...
	public static int updCmt(BoardCmtVO param) {
		String sql = "UPDATE t_board4_cmt "
				+ " SET m_dt = sysdate, r_dt = sysdate, cmt = ? "
